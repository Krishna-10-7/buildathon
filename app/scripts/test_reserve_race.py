"""SHIM - the suite lives in tests/test_reserve_race.py.
...
claim about the project that is no longer true.
  ...
  uv run python scripts/test_reserve_race.py   ->   pytest tests/test_reserve_race.py
...
raise SystemExit(pytest.main([
    str(ROOT / "tests" / "test_reserve_race.py"), "-q", "-p", "no:cacheprovider"]))
