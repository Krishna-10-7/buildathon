"""SHIM - the suite lives in tests/test_ratelimit.py.
...
claim about the project that is no longer true.
  ...
  uv run python scripts/test_ratelimit.py   ->   pytest tests/test_ratelimit.py
...
raise SystemExit(pytest.main([
    str(ROOT / "tests" / "test_ratelimit.py"), "-q", "-p", "no:cacheprovider"]))
